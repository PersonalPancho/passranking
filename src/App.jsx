...
function App() {
  ...
  // NUEVA FUNCIÓN: Borrar una contraseña específica
  const handleDeleteEntry = async (id) => {
    if (window.confirm("🗑️ ¿Estás seguro de eliminar esta contraseña del sistema?")) {
      const { error } = await supabase
        .from('ranking')
        .delete()
        .eq('id', id);
      ...
      // La lista se actualizará sola gracias al realtime de Supabase
    }
  };
    ...
    <div className="min-h-screen bg-black text-gray-200 p-6 md:p-12 selection:bg-green-500 selection:text-black">
      ...
      {!isLogged ? (
        <div className="flex flex-col items-center justify-center min-h-[80vh]">
          ...
          {/* Le pasamos isAdmin y onDelete al tablero incluso afuera */}
          {ranking.length > 0 && (
            <RankingBoard 
              ranking={ranking} 
        ...
        <div className="max-w-3xl mx-auto flex flex-col items-center">
          ...
          <div className="w-full flex justify-between items-center bg-gray-900 border border-gray-800 p-4 rounded-lg mb-8 shadow-md">
            <div className="font-mono">
              ...
              <span className={`${isAdmin ? 'text-red-500' : 'text-green-400'} font-bold`}>
                {nickname} {isAdmin && '(ADMIN)'}
              </span>
            </div>
            <div className="flex gap-4">
              {isAdmin && (
                <button 
                  ...
                  className="text-gray-500 hover:text-red-500 flex items-center gap-1 text-sm transition-colors font-bold"
